...
const me = {
    firstName: 'Alan',
    superHeroName: 'Vanilla Thunder', 
    homeTown: 'The Moon', 
    superPowers: ['flight', 'speedy', 'mind reading'], 
    superPowerXP: () => {
...
let setColor = faveColors => {
    faveColors = faveColors.splice(0, 3); 
...
let superPowersArr = me.superPowers;
let setPowers = superPowersArr => {
    for(let i = 0; i < superPowersArr.length; i++) {
        createLi(superPowersArr[i]); 
    }
};
