...
let iLove = (name, lovedItem) => {
	return `${name} loves ${lovedItem}`; 
}
...
let total = myNumbers.reduce((accum, element) => accum += element); 
...
const forTheLoveOfGeorge = notGeorge.map(element => 'George');
...
const totallyAwesome = people.reduce((accum, current) => accum + current.awesomeLevel);
